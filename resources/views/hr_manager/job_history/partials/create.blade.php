    ...
    <div class="pagetitle">
        ...
        <nav>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">Add Job History</li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
@endsection
...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Job History Details</h5>
            <!-- Floating Labels Form -->
            <form method="post" action="{{ route('hr_manager.job_history.store') }}" class="row g-3">
                @csrf
                <div class="col-md-6">
                        ...
                        <select class="form-select" name="job_grade_id" id="jobGrade" aria-label="Job Grade" required>
                            @if ($jobGrades->isNotEmpty())
                                ...
                                @foreach ($jobGrades as $jobGrade)
                                    <option value="{{ $jobGrade->id }}">{{ $jobGrade->grade }}</option>
                                @endforeach
                            @else
                                <option value="">--- Select a Job Grade ---</option>
                            @endif
