...
@section('title')
    <title>Request Leave Edit</title>
@endsection

@section('Page_title')
    <div class="pagetitle">
        <h1>Request Leave Edit</h1>
        <nav>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('hr_manager.dashboard') }}">Dashboard</a></li>
                <li class="breadcrumb-item"><a href="{{ route('hr_manager.leaves') }}">Leaves</a></li>
                <li class="breadcrumb-item active">Request Leave Edit</li>
            </ol>
        </nav>
    </div><!-- End Page Title -->
@endsection
...
@section('content')
    @include('hr_manager.leaves.partials.leave_info')
    <div class="text-end mb-2" title="Back to Employees">
        <a href="{{ route('hr_manager.leaves') }}" class="btn btn-primary"><i class="ri-arrow-left-s-line"></i></a>
    </div>
    <div class="card">
        <div class="card-body">
            <h5 class="card-title">Leave Details Edit</h5>
            <!-- Floating Labels Form -->
            <form method="post" class="row g-3" action="{{ route('hr_manager.leaves.update', $leave->id) }}">
                @csrf
                @method('put')
                <div class="col-md-12">
                        ...
                        <select class="form-select" name="leave_type_id" id="leave_type_id" aria-label="Leave Type"
                            ...
                            <option class="text-center" selected value="">-- Select Leave Type --</option>
                                @foreach ($leave_types as $leave_type)
                                    <option {{ $leave->leave_type_id == $leave_type->id ? 'selected' : '' }} value="{{ $leave_type->id }}">{{ $leave_type->name }}</option>
                                @endforeach
                            @else
                ...
                <div class="col-md-4">
                    <div class="form-floating">
                        <input type="text" class="form-control @error('start_date') is-invalid @enderror"
                            ...
                            value="{{ \Carbon\Carbon::parse($leave->start_date)->format('d-m-Y') ?? old('start_date') }}" required>
                ...
                <div class="col-md-4">
                    <div class="form-floating">
                        <input type="text" class="form-control @error('end_date') is-invalid @enderror" name="end_date"
                            id="end_datepicker" placeholder="End Date" autocomplete="off" value="{{ \Carbon\Carbon::parse($leave->end_date)->format('d-m-Y') ?? old('end_date') }}"
                            required>
                ...
                <div class="col-md-4">
                    <div class="form-floating">
                        <input type="number" class="form-control" name="total_days" value="{{ $leave->total_days ?? old('total_days') }}" id="total_days"
                ...
                <div class="col-12">
                    <div class="form-floating">
                        <textarea class="form-control" name="reason" placeholder="Reason" id="reason" style="height: 100px;" required>{{ $leave->reason ?? '' }}</textarea>
